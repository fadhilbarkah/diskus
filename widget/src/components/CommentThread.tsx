...
import type { Comment } from "./DiskusWidget";
...
export function CommentThread({
  ...
  useEffect(() => {
    try {
      ...
      alert(e.message || "Failed to update vote. Please try again.");
    }
  }, [comment.id]);
      ...
      if (success) {
        try {
          localStorage.setItem(
            "diskus_liked_comments",
            JSON.stringify([...likedComments, comment.id]),
          );
        } catch (e: any) {
          alert(e.message || "Failed to pin comment.");
        }
      } else {
        hasLiked.value = false;
        localLikesCount.value = Math.max(0, localLikesCount.value - 1);
  ...
  return (
    <div class={`relative ${depth > 0 ? "mt-6" : ""}`}>
      <div class="flex gap-3">
        {/* Avatar */}
        <div
          class={`w-8 h-8 rounded-full shrink-0 overflow-hidden bg-gray-100 dark:bg-gray-800 mt-1 border border-gray-200 dark:border-gray-700 relative z-10 ${isDeleted ? "opacity-40 grayscale" : ""}`}
        >
          <img
            src={dicebearUrl}
        ...
        <div class="flex-1 min-w-0 relative">
          <div class="mb-2 pr-6">
            <div class="flex items-center gap-2 min-w-0">
              <span class="font-bold text-[15px] text-gray-900 dark:text-gray-100 truncate block">
                {comment.authorName}
              </span>
              ...
              {isAuthor && !isDeleted && (
                <span class="bg-transparent border border-emerald-600/60 dark:border-emerald-500/50 text-emerald-700 dark:text-emerald-400 text-[11px] font-medium px-2 py-[1px] rounded-full shrink-0">
                  Author
                </span>
              )}
            </div>
            <div class="text-[13px] text-gray-500 dark:text-gray-400 mt-0.5 whitespace-nowrap">
          ...
          {widgetUser.value &&
            !isDeleted &&
            ((widgetUser.value.avatarSeed && widgetUser.value.avatarSeed === comment.avatarSeed) ||
              widgetUser.value.role === "admin" ||
              widgetUser.value.role === "user") && (
              <div class="absolute right-0 top-0">
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    globalActiveMenuId.value = showMenu ? null : comment.id;
                  }}
                ...
                {showMenu && (
                  <div
                    ...
                    class="absolute right-0 mt-1 w-32 bg-white dark:bg-[#222] border border-gray-200 dark:border-gray-700 shadow-lg rounded-lg py-1 z-20 text-sm"
                  >
                    {(widgetUser.value.role === "admin" || widgetUser.value.role === "user") &&
                      depth === 0 &&
                      onPin &&
                      comment.status === "approved" && (
                        <button
                          ...
                          class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-gray-800 text-gray-700 dark:text-gray-300 font-medium transition-colors"
                        >
                    ...
                    <button
                      ...
                      class="w-full text-left px-4 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 font-medium transition-colors"
                    >
                      Delete
                    </button>
                  </div>
          ...
          <div
            class="diskus-prose text-[15px] leading-relaxed text-gray-800 dark:text-gray-200 break-words"
          ...
          <div class="flex items-center gap-4 mt-3">
            {!isDeleted && (
              <button
                ...
                class={`flex items-center gap-1.5 text-[14px] ${hasLiked.value ? "text-red-500 dark:text-red-400" : "text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200"} transition-colors`}
              >
                <svg
                  class="w-[18px] h-[18px]"
                  fill={hasLiked.value ? "currentColor" : "none"}
                  stroke="currentColor"
                  viewBox="0 0 24 24"
                >
                  <path
            ...
            {!isDeleted && (
              <>
                ...
                <button
                  onClick={() => (globalActiveReplyId.value = isReplying ? null : comment.id)}
                  class="text-[14px] text-gray-500 dark:text-gray-400 hover:text-gray-900 dark:hover:text-gray-200 transition-colors"
                >
                  Reply
                </button>
              </>
