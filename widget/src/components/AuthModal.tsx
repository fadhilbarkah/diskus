...
import { globalShowAuthModal, globalAuthMode, globalAuthError, globalIsGuestReady, globalAuthReason, globalEnabledSocialLogins } from '../lib/auth';
...
export function AuthModal({ apiUrl, requireLogin }: Props) {
  ...
  const handleModalSubmit = async (e: Event) => {
    ...
    if (globalAuthMode.value === 'guest') {
      ...
      import('../lib/auth').then(async ({ globalResetToken }) => {
        if (!password.value || !globalResetToken.value) return (submitting.value = false, undefined);
  ...
  useEffect(() => {
    ...
    let expectedOrigin = '';
    try { expectedOrigin = new URL(apiUrl).origin; } catch {}
  ...
  const handleOAuth = (provider: string) => {
    ...
    window.open(url, `diskus_oauth_${provider}`, `width=${width},height=${height},left=${left},top=${top},popup=1`);
        ...
        {globalAuthMode.value !== 'invalid_reset_token' && (
          <div class="w-full min-[480px]:w-[55%] p-6 min-[480px]:p-8 min-[480px]:pt-12">
            <form onSubmit={handleModalSubmit} class="flex flex-col gap-4">
              ...
              {authSuccess.value && <div class="p-3 bg-green-50 text-green-600 dark:bg-green-900/20 dark:text-green-400 text-[13px] font-medium rounded-xl">{authSuccess.value}</div>}
              ...
              {(globalAuthMode.value === 'login' || globalAuthMode.value === 'register') && globalEnabledSocialLogins.value.length > 0 && (
                <div class="flex flex-col gap-3 mb-2">
                  <div class="flex gap-2 justify-center">
                    {globalEnabledSocialLogins.value.includes('google') && (
                      <button type="button" onClick={() => handleOAuth('google')} class="flex-1 py-2.5 flex justify-center items-center gap-2 border border-gray-200 dark:border-gray-700/80 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors bg-white dark:bg-[#1a1a1a]">
                    ...
                    {globalEnabledSocialLogins.value.includes('github') && (
                      <button type="button" onClick={() => handleOAuth('github')} class="flex-1 py-2.5 flex justify-center items-center gap-2 border border-gray-200 dark:border-gray-700/80 rounded-xl hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors bg-white dark:bg-[#1a1a1a]">
                        <svg class="w-5 h-5 text-gray-900 dark:text-white" fill="currentColor" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" clip-rule="evenodd"/></svg>
                      </button>
                    )}
                  </div>
                  <div class="relative flex items-center py-2">
                    <div class="flex-grow border-t border-gray-200 dark:border-gray-800"></div>
                    <span class="flex-shrink-0 mx-4 text-gray-400 dark:text-gray-500 text-[12px] font-medium">Or continue with email</span>
                    <div class="flex-grow border-t border-gray-200 dark:border-gray-800"></div>
                  </div>
                </div>
              )}
              ...
              {globalAuthMode.value !== 'reset_password' && globalAuthMode.value !== 'set_password' && (
                <div class="relative">
              ...
              {(globalAuthMode.value === 'login' || globalAuthMode.value === 'register' || globalAuthMode.value === 'reset_password' || globalAuthMode.value === 'set_password') && (
                <div class="relative">
              ...
              {globalAuthMode.value === 'login' && (
                <div class="flex justify-end mt-[-8px]">
                  <button type="button" onClick={() => { globalAuthMode.value = 'forgot_password'; authSuccess.value = ''; globalAuthError.value = ''; }} class="text-[13px] text-blue-600 dark:text-blue-400 font-medium hover:underline">Forgot password?</button>
                </div>
              )}
