      ...
      <div class="relative w-full min-w-0 max-w-[700px] max-h-[95vh] overflow-y-auto bg-white dark:bg-[#121212] border border-gray-200 dark:border-gray-800 rounded-2xl shadow-2xl flex flex-col min-[480px]:flex-row animate-in zoom-in-95 duration-200">
        ...
        <div class="w-full min-[480px]:w-[45%] bg-gray-50/50 dark:bg-[#161616] p-6 min-[480px]:p-8 flex flex-col justify-center border-b min-[480px]:border-b-0 min-[480px]:border-r border-gray-100 dark:border-gray-800/60">
            <div class="mt-2 min-[480px]:mt-0">
              <h3 class="text-[20px] font-bold text-gray-900 dark:text-white leading-tight">
                {globalAuthMode.value === 'login' ? 'Welcome back 👋' : globalAuthMode.value === 'register' ? 'Create account ✨' : globalAuthMode.value === 'forgot_password' ? 'Reset password 🔑' : globalAuthMode.value === 'reset_password' ? 'New password 🔐' : (globalAuthReason.value === 'like' ? 'Login required 🔒' : 'Leave a comment ✍️')}
              </h3>
              <p class="text-[14px] text-gray-500 dark:text-gray-400 mt-2.5">
                {globalAuthMode.value === 'login' ? (globalAuthReason.value === 'like' ? 'Sign in to like this comment.' : 'Sign in to leave a comment.') : globalAuthMode.value === 'register' ? 'Join our community.' : globalAuthMode.value === 'forgot_password' ? 'Enter your email to receive a reset link.' : globalAuthMode.value === 'reset_password' ? 'Enter your new password below.' : 'Fill in your details to comment as a guest.'}
              </p>
            </div>
            
            <div class="mt-8 flex flex-col gap-3 text-[14px]">
              {globalAuthMode.value !== 'login' && globalAuthMode.value !== 'reset_password' && (
              ...
              {globalAuthMode.value !== 'register' && globalAuthMode.value !== 'reset_password' && (
              ...
              {globalAuthMode.value !== 'guest' && !requireLogin && globalAuthReason.value !== 'like' && globalAuthMode.value !== 'reset_password' && (
        ...
        <div class="w-full min-[480px]:w-[55%] p-6 min-[480px]:p-8 min-[480px]:pt-12">
            <form onSubmit={handleModalSubmit} class="flex flex-col gap-4">
              ...
              {(globalAuthMode.value === 'login' || globalAuthMode.value === 'register' || globalAuthMode.value === 'reset_password') && (
                <div class="relative">
              ...
              <button type="submit" disabled={submitting.value} class="w-full mt-2 py-3 bg-blue-600 text-white text-[14px] font-semibold rounded-xl hover:bg-blue-700 transition-all disabled:opacity-50 disabled:cursor-not-allowed shadow-sm">
                  {submitting.value ? 'Please wait...' : globalAuthMode.value === 'login' ? 'Login' : globalAuthMode.value === 'register' ? 'Register' : globalAuthMode.value === 'forgot_password' ? 'Send reset link' : globalAuthMode.value === 'reset_password' ? 'Reset password' : 'Continue as Guest'}
              </button>
            </form>
        </div>
      </div>
