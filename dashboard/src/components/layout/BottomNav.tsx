...
import { Globe, LogOut, MessageSquare, Settings as SettingsIcon, Users } from "lucide-preact";
    ...
    <nav class="md:hidden fixed bottom-0 left-0 right-0 h-[4.5rem] pb-2 bg-white dark:bg-[#18181b] border-t border-gray-100 dark:border-gray-800 z-50 flex items-center justify-around px-2 shadow-[0_-4px_6px_-1px_rgba(0,0,0,0.05)]">
      {items.map((item) => {
        ...
        return (
          ...
          >
            <Icon
              class={`w-[22px] h-[22px] ${isActive ? "text-blue-600 stroke-[2.5]" : "stroke-2"}`}
            />
            <span
              class={`text-[10px] font-medium tracking-wide ${isActive ? "text-blue-600" : ""}`}
            >
              {item.label}
            </span>
          </button>
