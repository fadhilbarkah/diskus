...
export function TopHeader({ activePage, isCollapsed }: { activePage: Signal<string>, isCollapsed: Signal<boolean> }) {
  ...
  return (
    <header class={`fixed top-0 right-0 h-14 bg-white dark:bg-[#18181b] border-b border-gray-100 dark:border-gray-800 z-40 transition-all duration-300 ${isCollapsed.value ? 'left-0 md:left-16' : 'left-0 md:left-64'}`}>
          ...
          <div class="flex items-center justify-end shrink-0 h-full">
            {showSearch && (
              <div class="relative w-56 sm:w-64">
                ...
                />
                <span class="absolute right-3 top-1/2 -translate-y-1/2 text-[10px] text-gray-400 dark:text-gray-500 border border-gray-200 dark:border-gray-700 px-1.5 py-0.5 rounded bg-white dark:bg-[#27272a] font-medium hidden sm:inline-block shadow-sm">⌘K</span>
              </div>
            )}
          </div>
        </div>
