...
export function Sidebar({ activePage, isCollapsed }: { activePage: Signal<string>, isCollapsed: Signal<boolean> }) {
  return (
    <aside class={`hidden md:flex ${isCollapsed.value ? 'w-16' : 'w-64'} bg-white dark:bg-[#18181b] border-r border-gray-100 dark:border-gray-800 h-screen fixed left-0 top-0 flex-col font-sans z-50 transition-all duration-300 ease-out`}>
        ...
        <nav class={`flex-1 py-6 space-y-1.5 overflow-y-auto flex flex-col ${isCollapsed.value ? 'px-0 items-center' : 'px-4'}`}>
          {isCollapsed.value && (
        ...
        <div class={`p-4 mt-auto border-t border-gray-100 dark:border-gray-800 shrink-0 ${isCollapsed.value ? 'px-2' : ''}`}>
          <div class={`bg-white dark:bg-[#1f1f22] rounded-[20px] p-2 flex items-center justify-between transition-colors duration-200 ${isCollapsed.value ? 'flex-col gap-4 py-4 border-none bg-gray-50/80 dark:bg-[#1f1f22]' : 'border border-gray-200 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-[#27272a]'}`}>
            <button 
            ...
            >
              ...
              {!isCollapsed.value && (
                <div class="flex flex-col min-w-0">
                  <span class="text-sm font-bold text-gray-900 dark:text-gray-100 truncate max-w-[110px]" title={authState.user.value?.name || authState.user.value?.email}>
