...
import { ChevronDown, ChevronLeft, ChevronRight, MoreHorizontal, Pin, PinOff } from "lucide-preact";
    ...
    if (globalSearchQuery.value) {
      ...
      listToRender = listToRender.filter(
        (c) =>
          ...
          c.threadTitle?.toLowerCase().includes(q),
      );
    }
  ...
  const renderComments = () => {
    ...
    const renderNode = (c: any, depth: number = 0) => {
      ...
      const statusColors: Record<string, string> = {
        approved:
          "bg-green-50 dark:bg-green-900/30 text-green-600 dark:text-green-400 border-transparent",
        pending:
          "bg-gray-100 dark:bg-[#27272a] text-gray-600 dark:text-gray-400 border-transparent",
          ...
          >
            <div class="pt-1 shrink-0 z-10 bg-white dark:bg-[#18181b]">
              <input
                ...
                class="w-4 h-4 rounded border-gray-300 dark:border-gray-600 dark:bg-[#1f1f22] text-blue-600 focus:ring-blue-500 cursor-pointer"
              />
            </div>
                ...
                <div class="flex items-start justify-between mb-1 gap-2">
                  <div class="flex flex-col sm:flex-row sm:items-center gap-1 sm:gap-2 min-w-0">
                    <div class="flex items-center gap-2 flex-wrap min-w-0">
                      ...
                      {c.isAuthor && (
                        <span class="bg-blue-600 text-white text-[10px] font-bold px-2 py-0.5 rounded shrink-0">
                          Author
                        </span>
                      )}
                      {(c.threadTitle || c.threadKey) && (
                        <span
                          class="bg-gray-100 dark:bg-[#27272a] text-gray-500 dark:text-gray-400 text-[10px] font-medium px-2 py-0.5 rounded max-w-[200px] truncate"
                          title={c.threadTitle || c.threadKey}
                        >
                          {c.threadTitle || c.threadKey}
                        </span>
                      )}
                  ...
                  <div class="flex items-center gap-1 sm:gap-2 shrink-0">
                    <span
                      class={`border text-[10px] sm:text-xs px-2 py-0.5 sm:px-2.5 sm:py-1 rounded-full font-medium capitalize ${statusColors[c.status] || "bg-gray-50 dark:bg-gray-800 text-gray-600 dark:text-gray-400 border-gray-200 dark:border-gray-700"}`}
                    >
                      {c.status}
                    </span>
                    <div class="relative flex items-center">
                      ...
                      <button
                        onClick={() => (openMenuId.value = openMenuId.value === c.id ? null : c.id)}
                        class="text-gray-400 dark:text-gray-500 hover:text-gray-900 dark:hover:text-gray-100 p-1.5 rounded-md hover:bg-gray-100 dark:hover:bg-[#27272a] transition-colors cursor-pointer"
                      >
                      ...
                      {openMenuId.value === c.id && (
                        <div class="absolute right-0 top-full mt-1 w-36 bg-white dark:bg-[#18181b] border border-gray-200 dark:border-gray-800 shadow-lg rounded-xl py-1 z-20 text-sm">
                          {c.status !== "approved" && c.status !== "trash" && (
                            <button
                              onClick={() => handleSingleAction(c.id, "approved")}
                              class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-[#27272a] text-green-600 dark:text-green-400 font-medium cursor-pointer"
                            >
                              Approve
                            </button>
                          )}
                          {c.status !== "spam" && c.status !== "trash" && (
                            <button
                              onClick={() => handleSingleAction(c.id, "spam")}
                              class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-[#27272a] text-gray-700 dark:text-gray-300 font-medium cursor-pointer"
                            >
                              Mark as Spam
                            </button>
                          )}
                          {c.status === "trash" && (
                            <button
                              onClick={() => handleSingleAction(c.id, "approved")}
                              class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-[#27272a] text-blue-600 dark:text-blue-400 font-medium cursor-pointer"
                            >
                              Restore
                            </button>
                          )}
                          {c.status !== "trash" && (
                            <button
                              onClick={() => handleSingleAction(c.id, "trash")}
                              class="w-full text-left px-4 py-2 hover:bg-gray-50 dark:hover:bg-[#27272a] text-gray-700 dark:text-gray-300 font-medium cursor-pointer"
                            >
                              Move to Trash
                            </button>
                          ...
                          {c.status === "trash" && (
                            <button
                              onClick={() => handleSingleAction(c.id, "delete")}
                              class="w-full text-left px-4 py-2 hover:bg-red-50 dark:hover:bg-red-900/20 text-red-600 dark:text-red-400 font-bold border-t border-gray-100 dark:border-gray-800 mt-1 pt-2 cursor-pointer"
                            >
                              Delete Forever
                            </button>
                          )}
      ...
      <Card noPadding class="flex flex-col flex-1 min-h-0 border-gray-100 dark:border-gray-800">
              ...
              <span
                class={`text-xs px-2.5 py-0.5 rounded-full font-bold ${
                  activeTab.value === tab.id
                    ...
                    : "bg-gray-100 dark:bg-[#27272a] text-gray-500 dark:text-gray-400"
                }`}
              >
                {tab.count()}
              </span>
        ...
        <div class="px-4 py-4 flex items-center justify-between border-b border-gray-100 dark:border-gray-800">
          <div class="flex items-center gap-4">
            ...
            />
            <select
              class="border border-gray-200 dark:border-gray-700 rounded-lg text-sm px-3 py-2 bg-white dark:bg-[#18181b] outline-none font-medium text-gray-700 dark:text-gray-300 shadow-sm cursor-pointer"
              value={bulkAction.value}
              onChange={(e) => {
                bulkAction.value = (e.target as HTMLSelectElement).value;
                handleBulkAction();
              }}
            >
        ...
        <div class="flex-1 overflow-y-auto pb-4">
          {loading.value ? (
            ...
            <div class="flex flex-col">{renderComments()}</div>
          )}
        </div>
      </Card>
    </div>
