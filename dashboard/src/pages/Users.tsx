...
import { AlertTriangle, Loader2, RefreshCw, Trash2, Users as UsersIcon } from "lucide-preact";
      ...
      <Card noPadding class="overflow-hidden">
        {loading.value ? (
          ...
          <div class="flex flex-col items-center justify-center py-20 text-center px-4">
            ...
            <p class="text-gray-500 dark:text-gray-400 text-sm max-w-[250px] mb-8">
              There are no registered commenters yet.
            </p>
          </div>
        ) : (
            ...
            <table class="w-full text-left text-sm text-gray-600 dark:text-gray-300">
              ...
              <tbody class="divide-y divide-gray-100 dark:divide-gray-800">
                {users.value.map((user) => (
                  <tr
                    key={user.id}
                    class="hover:bg-gray-50/50 dark:hover:bg-[#1f1f22] transition-colors"
                  >
                    <td class="px-6 py-4">
                      <div class="flex items-center gap-3">
                        <div class="w-10 h-10 rounded-full bg-gray-100 dark:bg-[#18181b] flex items-center justify-center overflow-hidden shrink-0 border border-gray-200 dark:border-gray-700">
                          <img
                            src={`https://api.dicebear.com/10.x/thumbs/svg?seed=${encodeURIComponent(user.email)}`}
                            alt={user.name}
