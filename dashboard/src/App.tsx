...
export default function App() {
  ...
  return (
    <div class="flex min-h-screen font-sans bg-[#f8fafc] dark:bg-[#0f0f11]">
      {isDemo.value && !dismissBanner.value && (
        <div class="fixed top-0 left-0 right-0 z-[100] bg-blue-600 text-white px-4 py-2 flex items-center justify-center text-sm font-medium">
          <span>
            ...
            <a
              href="https://github.com/fadhilbarkah/diskus"
              target="_blank"
              rel="noopener noreferrer"
              class="underline hover:text-blue-100 ml-1"
            >
          ...
          <button
            onClick={() => {
              ...
              sessionStorage.setItem("diskus_demo_dismissed", "true");
            }}
      ...
      <main
        class={`flex-1 flex flex-col min-h-[calc(100vh-3.5rem)] mt-14 mb-[4.5rem] md:mb-0 w-full transition-all duration-300 ${isCollapsed.value ? "md:ml-16" : "md:ml-64"} ${isDemo.value && !dismissBanner.value ? "pt-8" : ""}`}
      >
        <div class="p-4 md:p-8 flex-1 overflow-y-auto">
          <div class="max-w-6xl mx-auto h-full">
            {activePage.value === "comments" ? (
              <Moderation />
            ) : activePage.value === "websites" ? (
              <Websites />
