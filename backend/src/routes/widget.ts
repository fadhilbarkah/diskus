...
widgetRoutes.post(
  ...
  rateLimitMiddleware(3, 60 * 60 * 1000),
  validate('json', z.object({ email: z.string().email().max(255), origin_url: z.string().url().max(1000) })),
...
widgetRoutes.post(
  ...
  validate('json', z.object({ token: z.string().max(500), newPassword: z.string().min(6).max(128) })),
