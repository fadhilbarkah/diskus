...
export class WidgetController {
  static async register(c: Context) {
    try {
      ...
      if (data._diskus_trap) {
        ...
        return c.json({
          token: "dummy_token_for_bots",
          user: { id: "dummy", email: data.email, name: data.name },
        });
      }
      ...
      return c.json({
        ...
        user: {
          ...
          name: newUser.name,
          role: "commenter",
          avatarSeed: hashEmail(newUser.email),
          isVerified: false,
        },
      });
    } catch (error: any) {
  ...
  static async login(c: Context) {
    ...
    if (
      dashboardUser &&
      (await WidgetService.verifyPassword(password, dashboardUser.passwordHash))
    ) {
      const token = await signToken({
        userId: dashboardUser.id,
        email: dashboardUser.email,
        name: dashboardUser.name || "Admin",
        role: dashboardUser.role,
        tokenVersion: dashboardUser.tokenVersion,
      });
      return c.json({
        token,
        user: {
          ...
          name: dashboardUser.name || "Admin",
          role: dashboardUser.role,
          avatarSeed: hashEmail(dashboardUser.email),
        },
      });
    }
    ...
    if (user && (await WidgetService.verifyPassword(password, user.passwordHash))) {
      ...
      return c.json({
        ...
        user: {
          ...
          name: user.name,
          role: "commenter",
          avatarSeed: hashEmail(user.email),
  ...
  static async resendVerification(c: Context) {
    ...
    if (user?.role !== "commenter") return c.json({ error: "Unauthorized" }, 401);
  ...
  static async verifyEmail(c: Context) {
    ...
    if (!success) {
      return c.html(
        "<h1>Verification failed</h1><p>The token is invalid or has already been used.</p>",
        400,
      );
    }
    ...
    return c.html(
      '<h1 style="font-family: sans-serif;">Email Verified</h1><p style="font-family: sans-serif;">You can now close this window and continue commenting.</p>',
    );
  }
  ...
  static async getComments(c: Context) {
    ...
    if (!auth.site)
      return c.json({ error: widgetAuthError(auth.failure), reason: auth.failure }, 403);
    ...
    const { comments, hasMore, total } = await WidgetService.getComments(
      site.id,
      threadKey,
      limit,
      offset,
    ...
    return c.json({
      ...
      hasMore,
      total,
      config: { requireLogin: site.requireLogin, enabledSocialLogins: site.enabledSocialLogins },
    });
  }
  ...
  static async postComment(c: Context<{ Variables: AuthVariables }>) {
    ...
    let initialStatus: "pending" | "approved" | "spam" | "trash" = site.requireModeration
      ? "pending"
      : "approved";
    ...
    if (site.enableEmail) {
      Promise.resolve().then(async () => {
        ...
        if (data.parentId && data.origin_url) {
          ...
          if (parentComment?.authorEmail && parentComment.authorEmail !== authorEmail) {
